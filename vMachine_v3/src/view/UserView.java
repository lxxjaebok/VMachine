...
public class UserView {
    ...
    public static void insertView() {
        ...
        if (charg_money % 1000 == 0 && charg_money > 0) {
            EmployeeDto dto = EmployeeDto.Of(password, name, tel, charg_money, on_off);
            int result = userService.insertData(dto);
            if (result != 0) {
                System.out.println("회원가입이 완료되었습니다.");
                EmployeeDto ch = userService.selectemp(password);
    ...
    public static void login() {
        ...
        EmployeeDto dto = userService.match(id, password);
        if (dto == null) {
    ...
    public static void insertCoin(int money) {
        // 회원 카드 금액 충전 view
        EmployeeDto nowMoney = userService.selectOne(nowId);
        int sum = nowMoney.charge_money() + money;
        int result = userService.setmoney(nowId, sum);
        if (result != 0) {
    ...
    public static void afterBuy() {
        //회원 제품 구매 view
        EmployeeDto user = userService.selectOne(nowId);
